...
const runtimes = {
    node () {
        const { readFile } = globalThis.require(`${'node'}:fs/promises`);
        const { inspect } = globalThis.require(`${'node'}:util`);
        const { resolve } = globalThis.require(`${'node'}:path`);
        const https = globalThis.require(`${'node'}:https`);
        const http = globalThis.require(`${'node'}:http`);
        function myfetch(path) {
            return new Promise((resolve, reject)=>{
                ...
                protocol.get(path, (res)=>{
                    ...
                    res.on('end', ()=>resolve(globalThis.Buffer.concat(chunks).toString('utf-8')));
                }).on('error', reject);
            });
        }
        return {
    ...
    browser () {
        return {
            ...
            async readFile (path) {
                const data = await globalThis.fetch(path);
    ...
    bun () {
        return {
                ...
                return new Promise((resolve, reject)=>{
                    (async ()=>{
                        try {
                            ...
                            fetch(path).then((res)=>res.text()).then(resolve).catch(reject);
                        }
                    })();
                });
            },
            cwd () {
                return globalThis.cwd;
            },
            exit (code) {
                globalThis.process.exit(code);
            },
            ...
            resolve (folder, path) {
                return globalThis.Bun.resolveSync(folder, path);
            },
            name: 'bun'
        };
    }
...
if (!globalThis.JSRuntime) {
    let runtime = 'browser';
    if (globalThis.process) runtime = 'node';
    if (globalThis.Deno) runtime = 'deno';
    if (globalThis.Bun) runtime = 'bun';
...
const props1 = new Properties(Runtime.loadProperties());
...
const props2 = new Properties(Primitive.loadProperties());
...
function NumberGetter(number) {
    ...
    if (memoData1.has(name)) return memoData1.get(name);
    const agaNumber = new AgalNumber(number);
    memoData1.set(name, agaNumber);
    return agaNumber;
}
...
const props3 = new Properties(Primitive.loadProperties());
...
function BooleanGetter(__boolean) {
    if (memoData2.has(__boolean)) {
        return memoData2.get(__boolean);
    }
    const booleanRuntime = new AgalBoolean();
    booleanRuntime.value = __boolean;
    memoData2.set(__boolean, booleanRuntime);
...
const mod3 = {
    AgalBooleanGetter: BooleanGetter,
    AgalBoolean: AgalBoolean,
    AgalNullValue: __default,
    AgalNull: AgalNull,
    AgalVoidValue: AgalVoid,
    AgalNumberGetter: NumberGetter,
    AgalNumber: AgalNumber,
    AgalPrimitive: Primitive,
    AgalStringGetter: StringGetter,
...
async function agal(code, path = JSRuntime.cwd() + '/inicio.agal', stack = defaultStack) {
    path = path.replace(/\\/g, '/');
