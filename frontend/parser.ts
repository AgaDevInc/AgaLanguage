...
export default class Parser {
  ...
  private parse_import_var(): Expr {
    ...
    const next = this.at();
    if (next.type !== TokenType.OpenParen && next.type !== TokenType.Dot)
      return this.makeError(
        { ...next, value: 'importar solo se puede usar en un ambito global' },
        ErrorType.ParserError
      ) as Expr;
  ...
  private parse_import_stmt(): Stmt {
    ...
    const data = this.expect(
      TokenType.String,
      'No se encontro la ruta del archivo'
    );
    ...
    if (this.at().type === TokenType.Con) {
      ...
      if (data.kind !== LITERALS_TYPE.OBJECT_LITERAL)
        return this.makeError(
          { ...tk, value: 'Se esperaba un objeto' },
          ErrorType.ParserError
        );
      stmt.with = data;
    }
  ...
  private parse_export_stmt(): Stmt {
    ...
    switch (data.kind) {
      ...
      case STATEMENTS_TYPE.VAR_DECLARATION:
        if (!data.value)
          return this.makeError(
            { ...this.at(), value: 'Se esperaba una asignación' },
            ErrorType.ParserError
          );
    ...
    if (!identifier)
      return this.makeError(
        { ...this.at(), value: 'Se esperaba un identificador' },
        ErrorType.ParserError
      );
    const value =
      data.kind === STATEMENTS_TYPE.VAR_DECLARATION
        ? data.value!
        : (data as unknown as Expr);
    const stmt: Stmt = {
  ...
  private parse_finally_stmt(isFN: boolean, isLoop: boolean) {
    ...
    if (start.type == TokenType.Error)
      return this.makeError(start, ErrorType.ParserError);
    const body: Stmt[] = [];
    ...
    if (end.type == TokenType.Error)
      return this.makeError(end, ErrorType.ParserError);
  ...
  private parse_catch_stmt(isFN: boolean, isLoop: boolean, strict = false) {
    ...
    if (type === TokenType.Capturar) {
      ...
      const error = this.expect(
        TokenType.Identifier,
        'No se encontro el identificador del error'
      );
      ...
      if (end.type == TokenType.Error)
        return this.makeError(end, ErrorType.ParserError);
      const next = this.parse_catch_stmt(isFN, isLoop) as
        | CatchStatement
        | ErrorStmt;
  ...
  private parse_try_stmt(
    ...
    const _catch = this.parse_catch_stmt(isFN, isLoop, true) as
      | CatchStatement
      | ErrorStmt;
  ...
  private parse_func_decl(isVar = false): FunctionDeclaration {
    ...
    while (this.not_eof() && this.at().type != TokenType.CloseParen) {
      if (this.at().type === TokenType.Dot) {
        const data = this.parse_iterable() as unknown as
          | ErrorStmt
          | IterableLiteral;
        if (data.kind === 'Error')
          return data as unknown as FunctionDeclaration;
        args.push(data);
    ...
    while (this.not_eof() && this.at().type != TokenType.CloseBrace) {
      ...
      if (data && data.kind === 'Error') return data as unknown as FunctionDeclaration;
      data && body.push(data);
    }
    const end = this.expect(TokenType.CloseBrace, 'No se encontró "}"');
    if (end.type == TokenType.Error)
    ...
    switch (tk.type) {
      ...
      case TokenType.Number: {
        ...
        if (tk.value.includes('$')) {
          ...
          if (2 > int || int > 36)
            return this.makeError(
              { ...tk, value: 'El número base debe estar entre 2 y 36' },
              ErrorType.ParserError
            ) as unknown as Expr;
          data[0] = int;
          data[1] = value;
        ...
        const value_ =
          data[0] === 10
            ? eval_complex(tk.value, {})
            : parseInt(data[1], data[0]);
      ...
      case TokenType.Funcion:
        return this.parse_func_decl(true) as unknown as Expr;
      case TokenType.Mientras:
